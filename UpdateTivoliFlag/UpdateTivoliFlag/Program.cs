...
using Mosaicco.UpdateTivoliFlag.Console.SPListWS;
using Mosaicco.UpdateTivoliFlag.Console.SPSiteWS;
using Mosaicco;
        ...
        static void Main(string[] args)
        {
            ...
            foreach (Tuple<string, string> WebItem in WebCollection)
                using (ListsSoapClient proxyList = new ListsSoapClient())
                {
                    ...
                    using (new OperationContextScope(proxyList.InnerChannel))
                    {
                        ...
                        foreach (System.Xml.XmlNode listItem in nl)
                        {
                            ...
                            if (!legalHoldStatus.Equals("Not On Hold"))
                            {
                                ...
                                // this if-statement was added for testing purposes only and should be removed from the final version of the console application
                                if ((caseSiteUrl.Contains("Test_Snow")) || (caseSiteUrl.Contains("Test2Case")))
                                {

                                    try
                                    {

                                        System.Console.WriteLine("Main() windows identify after impersonation: " + WindowsIdentity.GetCurrent().Name);
                                        System.Console.WriteLine("Attempting to update LegalHoldFlag for " + userAccount);

                                        //Update Legal Hold flag in AD
                                        mosaicUtility.UpdateLegalHoldStatus(displayName, legalHold);

                                    }
                                    catch (Exception ex)
                                    {
                                        System.Console.WriteLine("Error: Cannot update LegalHoldFlag for " + userAccount);
                                        mosaicUtility.LogToFile(ex.Message + " : Error: Cannot update LegalHoldFlag for " + userAccount);
                    ...
                    System.Console.WriteLine("UpdateTivoliFlag Console Application Done.");
